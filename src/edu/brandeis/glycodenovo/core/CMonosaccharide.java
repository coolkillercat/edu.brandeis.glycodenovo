...
public class CMonosaccharide {
	...
	public CMonosaccharide(String symNameClass, boolean permethylated) {
		for (CMonosaccharideSet mono : CMonosaccharideSet.values()) {
			if (symNameClass.equals(mono.symbol) || symNameClass.equals(mono.name)
					|| symNameClass.equals(mono.sacClass)) {
				mSymbol = mono.symbol;
				mName = mono.name;
				...
				if ( mPermethylated ) {
					...
					mVertices = new CMonosacVertices(mono.sacPermethylated.vertexComposition,
							mono.sacPermethylated.vertexPermethylated, mono.sacPermethylated.vertexMass);
				} else {
